...
public class FlightsByDest {
	...
	public boolean run(Job job)
		...
		job.setJarByClass(FlightsByDest.class);
		job.setJobName("FlightsByDest");

		job.setInputFormatClass(TextInputFormat.class);
		job.setMapperClass(FlightsByDestMapper.class);
		job.setReducerClass(FlightsByDestReducer.class);
	...
	public static void main(String[] argv) throws Exception {
		FlightsByDest fbc = prepare(argv);
		fbc.run(job);
	...
	public void printTotalUniqueAirlinesReport(BufferedReader br) throws IOException {
		System.out.println("Total unique destination:" + HdfsUtil.totalUnique(br));
	}

	public static FlightsByDest prepare(String[] argv) throws IOException {
		if (argv.length < 2) {
		...
		FlightsByDest fbc = new FlightsByDest();
		job = Job.getInstance(conf, "Flights by Destination");
